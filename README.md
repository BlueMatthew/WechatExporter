...
**BUG 1.8.0.7以前的版本异步加载方式存在一个比较严重的bug：当滚动到页面底部异步加载时，越靠后面的页码，加载的消息数量越少；页面打开全部消息异步加载时，消息只能加载到一半。如果iTunes备份还存在，请使用版本1.8.0.8重新导出一遍。如果过往的备份已经清除了，我后面会再出一个补丁程序，来修复已经导出的页面。**    
