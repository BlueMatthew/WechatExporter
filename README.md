...
本程序参考 https://github.com/stomakun/WechatExport-iOS 修改成C++来实现，主要是降低对dotnet的依赖，方便在MacOS下的运行。
...
libmp3lame需手动删除文件 include/libmp3lame.sym 中的行：lame_init_old
...
下载本代码的执行文件：https://github.com/BlueMatthew/WechatExport-iOS/releases/download/v1.5.0/Release_1.5.0_win.zip
解压压缩文件
执行解压出来的WechatExport.exe (如果运行报缺少必须的dll文件，请安装Visual C++ 2017 redist后再尝试运行)
按界面提示进行操作。
另外，解压目录下的res\templates子目录里存放了最终输出的聊天记录的html页面模版，通过两个%包含起来的字符串，譬如，%%NAME%%，不要修改之外，其它页面格式都可以自行调整。
